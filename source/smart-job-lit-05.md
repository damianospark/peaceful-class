...
<aside class="info">
**Excalidraw vs Miro 선택 가이드**:
- **가볍고 빠른 다이어그램** → Excalidraw (무료, 설치 불필요)
- **복잡한 협업 워크숍** → Miro (템플릿 풍부, 팀 기능 강력)
- **ChatGPT와 연동** → Excalidraw (Mermaid 문법 지원)
...
<aside class="info">
**Notion 팁**: Notion은 처음에는 복잡해 보이지만, "모든 것이 블록"이라는 개념만 이해하면 쉽습니다. 슬래시(/)를 누르면 추가할 수 있는 블록 목록이 나옵니다.
</aside>
...
<aside class="info">
**협업 팁**: 실제 팀 프로젝트에서는 팀원을 "Can edit" 권한으로 초대하세요. 외부 자문위원이나 멘토는 "Can comment" 권한으로 초대하면 피드백을 받을 수 있습니다.
</aside>
...
### 개선이 필요한 점
- [ ] 슬라이드 3: 플로우차트 단순화 → 완료
- [ ] 슬라이드 5: 시장 규모 데이터 출처 추가 필요
- [ ] 슬라이드 7: 경쟁사 비교표에 가격 정보 추가
...
<aside class="info">
**최종 메시지**: 
도구는 계속 변화하지만, **"생각을 정리하고 시각화하며 협업하는 능력"**은 평생 유용합니다. 오늘 배운 도구들이 언젠가 다른 것으로 대체되더라도, 이 과정에서 익힌 **사고방식과 워크플로우**는 여러분의 자산으로 남을 것입니다.
...
- [ ] **Slide 1**: 표지 (서비스명, 태그라인)
- [ ] **Slide 2**: 문제 정의 (3가지 핵심 문제)
- [ ] **Slide 3**: 솔루션 (우리가 제공하는 것)
- [ ] **Slide 4**: 작동 원리 (How it works)
- [ ] **Slide 5**: 시장 기회 (숫자와 그래프)
- [ ] **Slide 6**: 비즈니스 모델 (수익 구조)
- [ ] **Slide 7**: 경쟁 우위 (비교표)
- [ ] **Slide 8**: 실행 계획 (로드맵)
- [ ] **Slide 9**: 팀 소개
- [ ] **Slide 10**: Call to Action (연락처)
...
- [ ] 모든 산출물이 Notion에 정리되어 있는가?
- [ ] 외부 공유용 링크가 준비되었는가?
- [ ] 팀원 모두가 접근 권한을 가지고 있는가?
- [ ] 백업이 되어 있는가? (다운로드 또는 내보내기)
- [ ] 다음 프로젝트를 위한 회고 작성되었는가?
