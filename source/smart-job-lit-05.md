...
![사용자 플로우 다이어그램](./img/2025-11-12_00-41.png)

위와 같은 사용자 여정을 Excalidraw에서 시각화:
- 단계별로 사각형 또는 둥근 사각형으로 표현
- 아래로 흐르는 화살표로 연결
- 각 단계에서 분기가 있다면 다이아몬드 모양으로 결정 포인트 표시
- 주요 단계: 회원가입 → 매장 정보 입력 → 음악 스타일 선택 → AI 플레이리스트 생성 → 무료 체험 → 구독 결제 → 자동 재생
...
![수익 구조 다이어그램](./img/2025-11-12_00-37.png)

위와 같은 구조로 Excalidraw에서 다이어그램을 그립니다:
- 최상단: 소상공인 고객 (월 9,900원)
- 중간: 우리 서비스
- 하단: 4개 항목으로 분기 (AI 서버 비용 30%, 마케팅 비용 20%, 인건비 30%, 영업이익 20%)
